...
const Nav = () => {
  ...
  const menu = (
    <Menu>
      <Menu.Item style={{textAlign: "center"}} key="0">Account</Menu.Item>
      <Menu.Divider />
      <Menu.Item key="1">
        <div style={{ display: "flex" }}>
          ...
          <div style={{color: "#172b4d", fontSize: "1rem"}}>{user.nickname}</div>
          <span style={{color: "#b3bac5"}}>{user.email}</span>
          </div>
        </div>
      </Menu.Item>
      <Menu.Divider />
      <Menu.Item key="2"><a onClick={() => logout()}>Log Out</a></Menu.Item>
    </Menu>
  );
